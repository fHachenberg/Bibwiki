...
class SpecialSearch {
	...
	function goResult( $term ) {
		...
		//wpl $wgOut->addWikiText( wfMsg( 'noexactmatch', wfEscapeWikiText( $term ) ) );
	...
	function showResults( $term ) {
		global $wgDownloadsPath, $wgBibPath, $wgDownloadsUrl, $wgBibMessages; // wpl
		$something_found = false; //wpl
		...
		global $wgUser, $wgOut;
		$sk = $wgUser->getSkin();
		//wpl $wgOut->addWikiText( wfMsg( 'searchresulttext' ) );
		...
		if( $num || $this->offset ) {
			$prevnext = wfViewPrevNext( $this->offset, $this->limit,
				...
				wfArrayToCGI(
					...
					array( 'search' => $term ) ) );
			//wpl $wgOut->addHTML( "<br />{$prevnext}\n" );
		}
		
		$d = dir($wgBibPath);
		while (false !== ($f = $d->read())) {
		    if (preg_match('/\.bib$/',$f)) {
				$matches_in_bibfile = SearchBib($f, $term);
				if ($matches_in_bibfile != "") {
					$wgOut->addHTML($matches_in_bibfile);
		...
		$matches_in_papers = SearchPapers($wgDownloadsPath, $term, $wgDownloadsUrl);
		if ($matches_in_papers != "")
		{
			$wgOut->addHTML("<h2>".$wgBibMessages["search_h1_papers"]."</h2>");
			$wgOut->addHTML("<ol>");
			$wgOut->addHTML($matches_in_papers); 
			$wgOut->addHTML("</ol>");
			$something_found = true;
		}
		...
		if( $titleMatches ) {
			if( $titleMatches->numRows() ) {
				...
				//wpl $wgOut->addWikiText( '==' . wfMsg( 'notitlematches' ) . "==\n" );
			}
		}
		...
		if( $textMatches ) {
			if( $textMatches->numRows() ) {
				...
				//wpl $wgOut->addWikiText( '==' . wfMsg( 'notextmatches' ) . "==\n" );
			}
		}

		if ( $num == 0 && $something_found == false) {
			$wgOut->addWikiText( wfMsg( 'nonefound' ) );
		}
		if( $num || $this->offset ) {
			//wpl $wgOut->addHTML( "<p>{$prevnext}</p>\n" );
		}
		//wpl $wgOut->addHTML( $this->powerSearchBox( $term ) );
		wfProfileOut( $fname );
	}
